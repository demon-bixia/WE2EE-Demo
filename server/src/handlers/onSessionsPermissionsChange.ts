 ...
 */
async function onSessionsPermissionsChange(
  ...
  if (!associatedSession || !validSession(associatedSession)) {
    const message =
      "onSessionsPermissionsChange: The associated session is not a valid session";
    logger.err(message);
    return callbackResponse(callback, { status: "Not Found", message });
  }
  ...
  if (!associatedSession.main) {
    const message =
      "onSessionsPermissionsChange: Permissions can only be changed from the main session";
    logger.err(message);
    return callbackResponse(callback, { status: "Unauthorized", message });
  }
  ...
  if (!valid) {
    const message =
      "onSessionsPermissionChange: Invalid main session signature";
    logger.err(message);
    return callbackResponse(callback, { status: "Unauthorized", message });
  }
  ...
  if (
    ...
    const message =
      "onSessionsPermissionsChange: The user must have at least one main session";
    logger.err(message);
  ...
  if (!("username" in newUser)) {
    const message =
      "onSessionsPermissionsChange: Failed to update the user's session or keys";
    logger.err(message);
    return callbackResponse(callback, { status: "Bad Request", message });
  }
  ...
  logger.info(
    `onSessionsPermissionsChange: Changes permissions of session with #id ${payload.sessionId} to main: ${payload.main}`
  );
